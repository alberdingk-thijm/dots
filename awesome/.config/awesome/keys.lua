...
keys.globalkeys = gears.table.join(
    ...
    awful.key({ superkey, shiftkey   }, "k", function () awful.client.swap.byidx( -1)    end,
    ...
    awful.key({ superkey, shiftkey   }, "h",     function () awful.tag.incnmaster( 1, nil, true) end,
              {description = "increase the number of master clients", group = "layout"}),
    awful.key({ superkey, shiftkey   }, "l",     function () awful.tag.incnmaster(-1, nil, true) end,
              {description = "decrease the number of master clients", group = "layout"}),
    awful.key({ superkey, ctrlkey }, "h",     function () awful.tag.incncol( 1, nil, true)    end,
              {description = "increase the number of columns", group = "layout"}),
    awful.key({ superkey, ctrlkey }, "l",     function () awful.tag.incncol(-1, nil, true)    end,
    ...
    awful.key({ superkey }, "r",
              function()
                  awful.spawn.with_shell("rofi -modi run,drun -show drun -config ~/.config/rofi/themes/algonquin.rasi")
              end,
    ...
    -- Widgets popups
    -- awful.key({ superkey, altkey, }, "c", function () if beautiful.cal then beautiful.cal.show(7) end end,
    ...
    -- Spawn browser
    awful.key({ superkey, shiftkey }, "w", function() awful.spawn(browser, {floating = true}) end,
              {description = "web browser", group = "launcher"}),
    -- Spawn screen locker
    awful.key({ superkey, shiftkey    }, "l",      function () awful.spawn(locker) end,
              {description = "lock screen", group = "launcher"}),
    -- Spawn powermenu
    awful.key({ superkey, shiftkey }, "x", function() awful.spawn.with_shell("~/.local/bin/powermenu") end,
              {description = "power menu", group = "launcher"}),

    -- Copy primary to clipboard (terminals to gtk)
    awful.key({ superkey }, "c", function () awful.spawn.with_shell("xsel | xsel -i -b") end,
              {description = "copy terminal to gtk", group = "hotkeys"}),
    -- Copy clipboard to primary (gtk to terminals)
    awful.key({ superkey }, "v", function () awful.spawn.with_shell("xsel -b | xsel") end,
              {description = "copy gtk to terminal", group = "hotkeys"}),
    -- Media keys
    awful.key({ superkey }, "Insert", function () awful.spawn.with_shell("mpc toggle") end,
              {description = "play/pause mpd", group = "media"}),
    awful.key({ superkey }, "Delete", function () awful.spawn.with_shell("mpc stop") end,
              {description = "stop mpd", group = "media"}),
    awful.key({ superkey }, "End", function () awful.spawn.with_shell("mpc next") end,
              {description = "mpd next song", group = "media"}),
    awful.key({ superkey }, "Home", function () awful.spawn.with_shell("mpc prev") end,
              {description = "mpd previous song", group = "media"}),
    awful.key({ superkey }, "\\", function () awful.spawn.with_shell("eject -T") end,
              { description = "open/close CD drive", group = "media"})
)
